...
- バイナリファイルの検出は行わない
- すべてのファイルをテキストとして読み込み試行
- 読み取り時のエンコーディングエラーは無視（gpt-repository-loaderと同様の方式）
- 結果として、バイナリファイルは文字化けした内容または空の内容で出力される可能性がある
...
| ディレクトリの読み取り権限なし | 警告を出力、そのディレクトリをスキップ |
